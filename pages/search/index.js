...
const Search = ({ cars, params, loading, getMakes, makes }) => {
  ...
  const fetchPage = (i) => {
    if (active === "" || active === "all") {
      ...
      let data =
        active === "now" ? "buy_now=1" : active === "bid" ? "mmr_price=1" : "";
      const datas = {
        make: paramValue?.make || "",
        model: paramValue?.model || "",
        year: paramValue?.year || "",
    ...
    for (let number = leftSide; number <= rightSide; number++) {
      items.push(
        <div
          key={number}
          className={
            number === pageIndex ? "round-effect active1" : "round-effect"
          }
          onClick={() => {
            setPageIndex(number);
            fetchPage(number);
  ...
  const filterTab = (event) => {
    if (event !== active) {
      ...
      setActive(event);
      let data =
        event === "now" ? "buy_now=1" : event === "bid" ? "mmr_price=1" : "";
      const datas = {
        make: paramValue?.make || "",
        model: paramValue?.model || "",
        year: paramValue?.year || "",
  ...
  const getYearValue = () => {
    ...
    if (typeof data === "object") {
      let res = data.map((ele) => years.filter((option) => option == ele));
      setDefaultYear(
        [].concat(...res).map((ele) => {
          return {
            label: ele,
                    ...
                    {grid && (
                      <div
                        className={
                          ...
                          (open ? " gap-x-6 xl:gap-x-4" : " gap-x-4 xl:gap-x-2")
                        }
                        id="car-grid"
                      >
                        {data?.length > 0 &&
