        ...
        if (data) {
          ...
          if (Object.entries(data).length >= 1) {
            ...
            localStorage.setItem(
              "placeBidData",
              JSON.stringify(formatCollection.data[0])
            );
          }
        }
      })
      ...
      {id && (
        <>
          ...
          {carCollection?.length ? (
            <div
              className="blue-div px-20 mt-16 py-3 flex justify-between items-center"
              style={{ zIndex: "5" }}
            >
              <>
                <div key={selectedCollection._id} className="flex flex-1">
                  <div>
                    <h4 className="text-sm font-medium blue-text">
                      {selectedCollection.name}
                    </h4>
                    <h6 className="text-xs font-normal blue-text">
                      {selectedCollection?.vehicles?.length} cars selected
                    </h6>
                  </div>
                  <div className="flex py-2 ml-3 ">
                    {selectedCollection?.vehicles?.map((vehicle) => (
                      <img
