...
const Navbar = ({ beginLogin, beginLogout, userLoggedIn, total, cars }) => {
  ...
  const updateNotification = (id) => {
   console.log(id)
    ...
    fetch(
      enviroment.BASE_URL + `notification/${id}`,
      requestOptions
    )
      .then((response) => response.text())
      .then((result) => {
                    ...
                    >
                      <div className="px-4 py-3 bg-white w-full h-48 overflow-auto">
                        {notifications?.map((notification) =>
                          !notification?.read ? (
                            <div className="flex justify-between bg-white py-2" style={{borderBottom:"1px solid grey"}} onClick={()=>updateNotification(notification._id)} key={notification._id}>
                              <div
                                className=" text-sm"
                                style={{ cursor: "pointer" }}
