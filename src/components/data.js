//
// Filter inputs Options
//

export const SC = [
  { value: "SC 300", label: "SC 300" },
  { value: "SC 400", label: "SC 400" },
  { value: "SC 400 Luxury Sport Cpe", label: "SC 400 Luxury Sport Cpe" },
  { value: "SC 430", label: "SC 430" },
];
export const RC = [
  { value: "RC", label: "RC" },
  { value: "RC 200t", label: "RC 200t" },
  { value: "RC 300", label: "RC 300" },
  { value: "RC 350", label: "RC 350" },
  { value: "RC F", label: "RC F" },
];
export const NX = [
  { value: "NX", label: "NX" },
  { value: "NX 200t", label: "NX 200t" },
  { value: "NX 300h", label: "NX 300h" },
];
export const LS = [
  { value: "LS 400", label: "LS 400" },
  { value: "LS 400 Luxury Sdn", label: "LS 400 Luxury Sdn" },
  { value: "LS 430", label: "LS 430" },
  { value: "LS 460", label: "LS 460" },
  { value: "LS 600h L", label: "LS 600h L" },
];
export const LX = [
  { value: "LX 450 Luxury Wagon", label: "LX 450 Luxury Wagon" },
  { value: "LX 470 Luxury SUV", label: "LX 470 Luxury SUV" },
  { value: "LX 470 Luxury Wagon", label: "LX 470 Luxury Wagon" },
  { value: "LX 570", label: "LX 570" },
  { value: "LX 470", label: "LX 470" },
];
export const CLA = [
  { value: "CLA", label: "CLA" },
  { value: "CLA-Class", label: "CLA-Class" },
  { value: "CLK-Class", label: "CLK-Class" },
];
export const ACCORD = [
  { value: "Accord", label: "Accord" },
  { value: "Accord Coupe", label: "Accord Coupe" },
  { value: "Accord Crosstour", label: "Accord Crosstour" },
  { value: "Accord Hybrid", label: "Accord Hybrid" },
  { value: "Accord Sedan", label: "Accord Sedan" },
];
export const CRV = [
  { value: "CR-V", label: "CR-V" },
  { value: "CR-V Hybrid", label: "CR-V Hybrid" },
  { value: "CR-Z", label: "CR-Z" },
];
export const CIVIC = [
  { value: "Civic", label: "Civic" },
  { value: "Civic Coupe", label: "Civic Coupe" },
  { value: "Civic Hatchback", label: "Civic Hatchback" },
  { value: "Civic Hybrid", label: "Civic Hybrid" },
  { value: "Civic Sedan", label: "Civic Sedan" },
  { value: "Civic Si", label: "Civic Si" },
  { value: "Civic Si Coupe", label: "Civic Si Coupe" },
  { value: "Civic Si Sedan", label: "Civic Si Sedan" },
  { value: "Civic Type R", label: "Civic Type R" },
];
export const CLS = [
  { value: "CLS-Class", label: "CLS-Class" },
  { value: "CLS", label: "CLS" },
];
export const GLA = [
  { value: "GLA", label: "GLA" },
  { value: "GLA-Class", label: "GLA-Class" },
];
export const GL = [
  { value: "GL", label: "GL" },
  { value: "GL-Class", label: "GL-Class" },
];
export const IS = [
  { value: "IS", label: "IS" },
  { value: "IS 200T", label: "IS 200T" },
  { value: "IS 250", label: "IS 250" },
  { value: "IS 250C", label: "IS 250C" },
  { value: "IS 300", label: "IS 300" },
  { value: "IS 350", label: "IS 350" },
  { value: "IS 350C", label: "IS 350C" },
  { value: "IS F", label: "IS F" },
];
export const GS = [
  { value: "GS 200t", label: "GS 200t" },
  { value: "GS 300", label: "GS 300" },
  { value: "GS 300 Luxury Perform Sdn", label: "GS 300 Luxury Perform Sdn" },
  { value: "GS 350", label: "GS 350" },
  { value: "GS 400 Luxury Perform Sdn", label: "GS 400 Luxury Perform Sdn" },
  { value: "GS 430", label: "GS 430" },
  { value: "GS 450h", label: "GS 450h" },
  { value: "GS F", label: "GS F" },
];
export const ES = [
  { value: "ES 300", label: "ES 300" },
  { value: "ES 300 Luxury Sport Sdn", label: "ES 300 Luxury Sport Sdn" },
  { value: "ES 300h", label: "ES 300h" },
  { value: "ES 330", label: "ES 330" },
  { value: "ES 350", label: "ES 350" },
];
export const GX = [
  { value: "GX 460", label: "GX 460" },
  { value: "GX 470", label: "GX 470" },
];
export const RX = [
  { value: "RX 450h", label: "RX 450h" },
  { value: "RX 400h", label: "RX 400h" },
  { value: "RX 350", label: "RX 350" },
  { value: "RX 330", label: "RX 330" },
  { value: "RX 300 Luxury SUV", label: "RX 300 Luxury SUV" },
];
export const FuelType = [
  { value: "Gasoline", label: "Gasoline" },
  { value: "Diesel", label: "Diesel" },
  { value: "Alternative/Hybrid", label: "Alternative/Hybrid" },
  { value: "Ethanol", label: "Ethanol" },
  { value: "Flex Fuel Capability", label: "Flex Fuel Capability" },
  { value: "Gas/Electric Hybrid", label: "Gas/Electric Hybrid" },
  { value: "Hybrid", label: "Hybrid" },
  { value: "Not Specified", label: "Not Specified" },
];
export const BodyType = [
  { value: "Sedan", label: "Sedan" },
  { value: "Suv", label: "Suv" },
  { value: "Van", label: "Van" },
  { value: "Truck", label: "Truck" },
  { value: "Wagon", label: "Wagon" },
];
export const TransmissionType = [
  { value: "Automatic", label: "Automatic" },
  { value: "Manual", label: "Manual" },
  { value: "CVT", label: "CVT" },
  { value: "Not Specified", label: "Not Specified" },
];
export const EngineType = [
  { value: "1 Cylinder", label: "1 Cylinder" },
  { value: "2 Cylinder", label: "2 Cylinder" },
  { value: "3 Cylinder", label: "3 Cylinder" },
  { value: "3 Cylinder Turbo", label: "3 Cylinder Turbo" },
  { value: "4 Cylinder", label: "4 Cylinder" },
  { value: "4 Cylinder Turbo", label: "4 Cylinder Turbo" },
  { value: "5 Cylinder", label: "5 Cylinder" },
  { value: "5 Cylinder Turbo", label: "5 Cylinder Turbo" },
  { value: "6 Cylinder", label: "6 Cylinder" },
  { value: "6 Cylinder Turbo", label: "6 Cylinder Turbo" },
  { value: "8 Cylinder", label: "8 Cylinder" },
  { value: "8 Cylinder Turbo", label: "8 Cylinder Turbo" },
  { value: "10 Cylinder", label: "10 Cylinder" },
  { value: "12 Cylinder", label: "12 Cylinder" },
  { value: "Rotary", label: "Rotary" },
  { value: "Not Specified", label: "Not Specified" },
];
export const MIN = [
  { value: 10000, label: "10,000" },
  { value: 20000, label: "20,000" },
  { value: 30000, label: "30,000" },
  { value: 40000, label: "40,000" },
  { value: 50000, label: "50,000" },
  { value: 60000, label: "60,000" },
  { value: 70000, label: "70,000" },
  { value: 80000, label: "80,000" },
  { value: 90000, label: "90,000" },
  { value: 100000, label: "100,000" },
  { value: 110000, label: "110,000" },
  { value: 120000, label: "120,000" },
  { value: 130000, label: "130,000" },
  { value: 140000, label: "140,000" },
];
export const MAX = [
  { value: 200000, label: "200,000" },
  { value: 190000, label: "190,000" },
  { value: 180000, label: "180,000" },
  { value: 170000, label: "170,000" },
  { value: 160000, label: "160,000" },
  { value: 150000, label: "150,000" },
  { value: 140000, label: "140,000" },
  { value: 130000, label: "130,000" },
  { value: 120000, label: "120,000" },
  { value: 110000, label: "110,000" },
  { value: 100000, label: "100,000" },
  { value: 90000, label: "90,000" },
  { value: 80000, label: "80,000" },
  { value: 70000, label: "70,000" },
  { value: 60000, label: "60,000" },
  { value: 50000, label: "50,000" },
  { value: 40000, label: "40,000" },
  { value: 30000, label: "30,000" },
  { value: 20000, label: "20,000" },
  { value: 10000, label: "10,000" },
];
export const InteriorColour = [
  { value: "Black", label: "Black" },
  { value: "White", label: "White" },
  { value: "Brown", label: "Brown" },
  { value: "Burgundy", label: "Burgundy" },
  { value: "Charcoal", label: "Charcoal" },
  { value: "Gold", label: "Gold" },
  { value: "Gray", label: "Gray" },
  { value: "Green", label: "Green" },
  { value: "Off-white", label: "Off-white" },
  { value: "Orange", label: "Orange" },
  { value: "Purple", label: "Purple" },
  { value: "Red", label: "Red" },
  { value: "Silver", label: "Silver" },
  { value: "Tan", label: "Tan" },
  { value: "Turquoise", label: "Turquoise" },
  { value: "Yellow", label: "Yellow" },
  { value: "Not Specified", label: "Not Specified" },
];
export const InteriorType = [
  { value: "Cloth", label: "Cloth" },
  { value: "Leather", label: "Leather" },
  { value: "Vinyl", label: "Vinyl" },
  { value: "Not Specified", label: "Not Specified" },
];
export const ExternalColour = [
  { value: "Beige", label: "Beige" },
  { value: "Black", label: "Black" },
  { value: "Blue", label: "Blue" },
  { value: "White", label: "White" },
  { value: "Brown", label: "Brown" },
  { value: "Burgundy", label: "Burgundy" },
  { value: "Lime", label: "Lime" },
  { value: "Gold", label: "Gold" },
  { value: "Gray", label: "Gray" },
  { value: "Green", label: "Green" },
  { value: "Off-white", label: "Off-white" },
  { value: "Orange", label: "Orange" },
  { value: "Purple", label: "Purple" },
  { value: "Red", label: "Red" },
  { value: "Silver", label: "Silver" },
  { value: "Turquoise", label: "Turquoise" },
  { value: "Yellow", label: "Yellow" },
  { value: "Not Specified", label: "Not Specified" },
];
export const FacilitationLocation = [
  { value: "myCentralAuction", label: "myCentralAuction" },
  {
    value: "AL - America's Auto Auction Birmingham",
    label: "AL - America's Auto Auction Birmingham",
  },
  {
    value: "AL - Auto Auction of Montgomery",
    label: "AL - Auto Auction of Montgomery",
  },
  {
    value: "AL - Dealers Auto Auction - Huntsville",
    label: "AL - Dealers Auto Auction - Huntsville",
  },
  {
    value: "AL - Dealers Auto Auction - Mobile",
    label: "AL - Dealers Auto Auction - Mobile",
  },
  {
    value: "AL - Shoals North Alabama Auto Auction",
    label: "AL - Shoals North Alabama Auto Auction",
  },
  {
    value: "AZ - Metro Auto Auction Phoenix",
    label: "AZ - Metro Auto Auction Phoenix",
  },
  { value: "CA - ADESA Brashers", label: "CA - ADESA Brashers" },
  {
    value: "CA - All Valley Dealer Auto Auction",
    label: "CA - All Valley Dealer Auto Auction",
  },
  {
    value: "CA - Auctions In Motion - Thousand Oaks",
    label: "CA - Auctions In Motion - Thousand Oaks",
  },
  {
    value: "CA - AutoNation Auto Auction",
    label: "CA - AutoNation Auto Auction",
  },
  {},
  {},
  {
    value: "CA - Norwalk Auto Auction",
    label: "CA - Norwalk Auto Auction",
  },
  {
    value: "CA - South Bay Auto Auction",
    label: "CA - South Bay Auto Auction",
  },
  {
    value: "FL - AutoNation Auto Auction - Orlando",
    label: "FL - AutoNation Auto Auction - Orlando",
  },
  {
    value: "FL - Dealers Auction Xchange LLC",
    label: "FL - Dealers Auction Xchange LLC",
  },
  {},
  {},
  {},
  {},
  {
    value: "FL - Orlando Longwood Auto Auction",
    label: "FL - Orlando Longwood Auto Auction",
  },
  {
    value: "FL - South Florida Auto Auction",
    label: "FL - South Florida Auto Auction",
  },
  {
    value: "FL - Space Coast Auto Auction of Melbourne",
    label: "FL - Space Coast Auto Auction of Melbourne",
  },
  {
    value: "FL - Tallahassee Auto Auction",
    label: "FL - Tallahassee Auto Auction",
  },
  {
    value: "GA - Albany Auto Auction Inc",
    label: "GA - Albany Auto Auction Inc",
  },
  {
    value: "GA - America's Auto Auction - Atlanta",
    label: "GA - America's Auto Auction - Atlanta",
  },
  {
    value: "GA - AutoNation Auto Auction - Atlanta",
    label: "GA - AutoNation Auto Auction - Atlanta",
  },
  {
    value: "GA - Georgia-Carolina Auto Auction",
    label: "GA - Georgia-Carolina Auto Auction",
  },
  { value: "GA - Rome Auto Auction", label: "GA - Rome Auto Auction" },
  {
    value: "IA - Dealers Choice Auto Auction - Mediapolis",
    label: "IA - Dealers Choice Auto Auction - Mediapolis",
  },
  {
    value: "IA - Des Moines Auto Auction",
    label: "IA - Des Moines Auto Auction",
  },
  { value: "IA - Plaza Auto Auction", label: "IA - Plaza Auto Auction" },
  {
    value: "ID - Dealers Auto Auction Of Idaho",
    label: "ID - Dealers Auto Auction Of Idaho",
  },
  {
    value: "IL - America's Auto Auction - Chicago",
    label: "IL - America's Auto Auction - Chicago",
  },
  {
    value: "IL - America's Auto Auction Saint Louis",
    label: "IL - America's Auto Auction Saint Louis",
  },
  {
    value: "IL - Greater Rockford Auto Auction Inc",
    label: "IL - Greater Rockford Auto Auction Inc",
  },
  {
    value: "IN - Auto Dealers Exchange",
    label: "IN - Auto Dealers Exchange",
  },
  {
    value: "IN - Chicagoland Independent Auction",
    label: "IN - Chicagoland Independent Auction",
  },
  {
    value: "IN - Clark County Auto Auction",
    label: "IN - Clark County Auto Auction",
  },
  {
    value: "IN - Greater Mishawaka Auto Auction",
    label: "IN - Greater Mishawaka Auto Auction",
  },
  {
    value: "IN - Indiana Auto Auction",
    label: "IN - Indiana Auto Auction",
  },
  {},
  {
    value: "IN - WOLFE'S EVANSVILLE AUTO AUCTION",
    label: "IN - WOLFE'S EVANSVILLE AUTO AUCTION",
  },
  {
    value: "KS - Mid Kansas Auto Auction Inc",
    label: "KS - Mid Kansas Auto Auction Inc",
  },
  {
    value: "KY - America's Auto Auction Bowling Green",
    label: "KY - America's Auto Auction Bowling Green",
  },
  {
    value: "LA - America's Auto Auction Baton Rouge",
    label: "LA - America's Auto Auction Baton Rouge",
  },
  {
    value: "LA - Greater Shreveport Bossier Auto Auction",
    label: "LA - Greater Shreveport Bossier Auto Auction",
  },
  {
    value: "LA - Lake Charles Auto Auction",
    label: "LA - Lake Charles Auto Auction",
  },
  {
    value: "LA - Louisiana's 1st Choice Auto Auction",
    label: "LA - Louisiana's 1st Choice Auto Auction",
  },
  {},
  { value: "MA - Adesa Boston", label: "MA - Adesa Boston" },
  {
    value: "MA - Central Mass. Auto Auction",
    label: "MA - Central Mass. Auto Auction",
  },
  {
    value: "MA - Lynnway Auto Auction",
    label: "MA - Lynnway Auto Auction",
  },
  {},
  {
    value: "MD - Bel Air Auto Auction",
    label: "MD - Bel Air Auto Auction",
  },
  {},
  {
    value: "ME - Port City Auto Auction",
    label: "ME - Port City Auto Auction",
  },
  {
    value: "MI - America's Auto Auction West Michigan",
    label: "MI - America's Auto Auction West Michigan",
  },
  {
    value: "MI - Americas Auto Auction I94, Inc",
    label: "MI - Americas Auto Auction I94, Inc",
  },
  {
    value: "MI - Fastlane Auto Exchange",
    label: "MI - Fastlane Auto Exchange",
  },
  {
    value: "MI - Grand Rapids Auto Auction",
    label: "MI - Grand Rapids Auto Auction",
  },
  {
    value: "MI - Greater Detroit Auto Auction",
    label: "MI - Greater Detroit Auto Auction",
  },
  {
    value: "MI - Greater Kalamazoo Auto",
    label: "MI - Greater Kalamazoo Auto",
  },
  {
    value: "MI - West Michigan Auto Auction",
    label: "MI - West Michigan Auto Auction",
  },
  {},
  {},
  {
    value: "MN - Mid-State Auto Auction",
    label: "MN - Mid-State Auto Auction",
  },
  { value: "MO - 166 Auto Auction", label: "MO - 166 Auto Auction" },
  {
    value: "MO - America's Auto Auction - Kansas City",
    label: "MO - America's Auto Auction - Kansas City",
  },
  {},
  {},
  {
    value: "MO - Missouri Auto Auction",
    label: "MO - Missouri Auto Auction",
  },
  {
    value: "MS - Dealers Auto Auction - Jackson",
    label: "MS - Dealers Auto Auction - Jackson",
  },
  {},
  {
    value: "MT - Auto Auction Of Montana",
    label: "MT - Auto Auction Of Montana",
  },
  {
    value: "NC - Greenville Auto Auction",
    label: "NC - Greenville Auto Auction",
  },
  {},
  {},
  {
    value: "NC - Speedway Auto Auction of Charlotte",
    label: "NC - Speedway Auto Auction of Charlotte",
  },
  {
    value: "NH - Auto Auction Of New England",
    label: "NH - Auto Auction Of New England",
  },
  {},
  {
    value: "NV - Dealers Auto Auction of Las Vegas",
    label: "NV - Dealers Auto Auction of Las Vegas",
  },
  {
    value: "NY - Buffalo Auto Auction Incorporated",
    label: "NY - Buffalo Auto Auction Incorporated",
  },
  {
    value: "NY - Rochester Syracuse Auto Auction",
    label: "NY - Rochester Syracuse Auto Auction",
  },
  {
    value: "NY - State Line Auto Auction",
    label: "NY - State Line Auto Auction",
  },
  { value: "OH - Akron Auto Auction", label: "OH - Akron Auto Auction" },
  {
    value: "OH - America's Auto Auction Toledo",
    label: "OH - America's Auto Auction Toledo",
  },
  {
    value: "OH - Columbus Fair Auto Auction",
    label: "OH - Columbus Fair Auto Auction",
  },
  {
    value: "OH - Montpelier Auto Auction",
    label: "OH - Montpelier Auto Auction",
  },
  {
    value: "OH - Skipco Auto Auction",
    label: "OH - Skipco Auto Auction",
  },
  { value: "OH - Value Auto Auction", label: "OH - Value Auto Auction" },
  {
    value: "OK - America's Auto Auction - Tulsa",
    label: "OK - America's Auto Auction - Tulsa",
  },
  { value: "OK - Daa Oklahoma City", label: "OK - Daa Oklahoma City" },
  { value: "OK - Daa Oklahoma City", label: "OK - Daa Oklahoma City" },
  { value: "OK - Z66 Auto Auction", label: "OK - Z66 Auto Auction" },
  {
    value: "PA - America's Auto Auction - Harrisburg",
    label: "PA - America's Auto Auction - Harrisburg",
  },
  {
    value: "PA - America's Auto Auction - Pittsburgh",
    label: "PA - America's Auto Auction - Pittsburgh",
  },
  {
    value: "PA - America's Auto Auction Lancaster",
    label: "PA - America's Auto Auction Lancaster",
  },
  {
    value: "PA - Bloomsburg Auto Auction",
    label: "PA - Bloomsburg Auto Auction",
  },
  {
    value: "PA - Central Pennsylvania Auto Auction",
    label: "PA - Central Pennsylvania Auto Auction",
  },
  {
    value: "PA - Corry Auto Dealers Exchange",
    label: "PA - Corry Auto Dealers Exchange",
  },
  {
    value: "PA - Greater Erie Auto Auction",
    label: "PA - Greater Erie Auto Auction",
  },
  {
    value: "PA - Lehigh Valley Auto Auction",
    label: "PA - Lehigh Valley Auto Auction",
  },
  {},
  {},
  {
    value: "SC - America's Auto Auction - Greenville",
    label: "SC - America's Auto Auction - Greenville",
  },
  {
    value: "SC - Carolina Auto Auction Inc",
    label: "SC - Carolina Auto Auction Inc",
  },
  {
    value: "SC - Charleston Auto Auction",
    label: "SC - Charleston Auto Auction",
  },
  {
    value: "SC - Grand Strand Auto Auction LLC",
    label: "SC - Grand Strand Auto Auction LLC",
  },
  { value: "SC - Rawls Auto Auction", label: "SC - Rawls Auto Auction" },
  {
    value: "TN - Airport Auto Auction",
    label: "TN - Airport Auto Auction",
  },
  {
    value: "SC - Charleston Auto Auction",
    label: "SC - Charleston Auto Auction",
  },
  {
    value: "SC - Grand Strand Auto Auction LLC",
    label: "SC - Grand Strand Auto Auction LLC",
  },
  { value: "SC - Rawls Auto Auction", label: "SC - Rawls Auto Auction" },
  {
    value: "TN - Airport Auto Auction",
    label: "TN - Airport Auto Auction",
  },
  {
    value: "TN - Dealers Auto Auction - Memphis",
    label: "TN - Dealers Auto Auction - Memphis",
  },
  {
    value: "TN - Dealers Auto Auction - Murfreesboro",
    label: "TN - Dealers Auto Auction - Murfreesboro",
  },
  {
    value: "TN - Dealers Auto Auction Chattanooga",
    label: "TN - Dealers Auto Auction Chattanooga",
  },
  {
    value: "TN - Dealers Auto Auction East Tennessee LLC",
    label: "TN - Dealers Auto Auction East Tennessee LLC",
  },
  {
    value: "TN - Music City Auto Auction",
    label: "TN - Music City Auto Auction",
  },
  { value: "TN - Stream", label: "TN - Stream" },
  {
    value: "TX - Alliance Auto Auction - Austin",
    label: "TX - Alliance Auto Auction - Austin",
  },
  {
    value: "TX - Alliance Auto Auction Dallas",
    label: "TX - Alliance Auto Auction Dallas",
  },
  {
    value: "TX - Alliance Auto Auction of Abilene",
    label: "TX - Alliance Auto Auction of Abilene",
  },
  {
    value: "TX - Alliance Auto Auction of Longview",
    label: "TX - Alliance Auto Auction of Longview",
  },
  {
    value: "TX - Alliance Auto Auction of Waco",
    label: "TX - Alliance Auto Auction of Waco",
  },
  {
    value: "TX - America's Auto Auction - Austin/San",
    label: "TX - America's Auto Auction - Austin/San",
  },
  {
    value: "TN - Memphis Auto Auction",
    label: "TN - Memphis Auto Auction",
  },
  {
    value: "TN - Music City Auto Auction",
    label: "TN - Music City Auto Auction",
  },
  { value: "TN - Stream", label: "TN - Stream" },
  {
    value: "TX - Alliance Auto Auction - Austin",
    label: "TX - Alliance Auto Auction - Austin",
  },
  {
    value: "TX - Alliance Auto Auction Dallas",
    label: "TX - Alliance Auto Auction Dallas",
  },
  {
    value: "TX - Alliance Auto Auction of Abilene",
    label: "TX - Alliance Auto Auction of Abilene",
  },
  {
    value: "TX - Alliance Auto Auction of Longview",
    label: "TX - Alliance Auto Auction of Longview",
  },
  {
    value: "TX - Alliance Auto Auction of Waco",
    label: "TX - Alliance Auto Auction of Waco",
  },
  {
    value: "TX - America's Auto Auction - Austin/San Antonio",
    label: "TX - America's Auto Auction - Austin/San Antonio",
  },
  {
    value: "TX - America's Auto Auction - Dallas",
    label: "TX - America's Auto Auction - Dallas",
  },
  {
    value: "TX - America's Auto Auction - North Houston",
    label: "TX - America's Auto Auction - North Houston",
  },
  {
    value: "TX - America's Auto Auction Houston",
    label: "TX - America's Auto Auction Houston",
  },
  {
    value: "TX - AutoNation Auto Auction - Houston",
    label: "TX - AutoNation Auto Auction - Houston",
  },
  {
    value: "TX - Big Valley Auto Auction",
    label: "TX - Big Valley Auto Auction",
  },
  {
    value: "TX - Corpus Christi Auto Auction",
    label: "TX - Corpus Christi Auto Auction",
  },
  { value: "TX - DAX of Rockwall", label: "TX - DAX of Rockwall" },
  {
    value: "TX - El Paso Independent Auto Auction",
    label: "TX - El Paso Independent Auto Auction",
  },
  {
    value: "TX - Greater Tyler Auto Auction",
    label: "TX - Greater Tyler Auto Auction",
  },
  {
    value: "TX - Houston Auto Auction",
    label: "TX - Houston Auto Auction",
  },
  {
    value: "TX - Lone Star Auto Auction Inc",
    label: "TX - Lone Star Auto Auction Inc",
  },
  {
    value: "TX - Lubbock Auto Auction",
    label: "TX - Lubbock Auto Auction",
  },
  {},
  {},
  {},
  {
    value: "TX - Metro Auto Auction Dallas",
    label: "TX - Metro Auto Auction Dallas",
  },
  {
    value: "TX - San Antonio Auto Auction",
    label: "TX - San Antonio Auto Auction",
  },
  {
    value: "TX - Texas Lone Star Auto Auction",
    label: "TX - Texas Lone Star Auto Auction",
  },
  {
    value: "VA - America's Auto Auction Virginia",
    label: "VA - America's Auto Auction Virginia",
  },
  {},
  {},
  {
    value: "VA - Richmond Auto Auction",
    label: "VA - Richmond Auto Auction",
  },
  { value: "WA - DAA Northwest", label: "WA - DAA Northwest" },
  { value: "WA - DAA Seattle", label: "WA - DAA Seattle" },
  {
    value: "WI - Badger State Auto Auction",
    label: "WI - Badger State Auto Auction",
  },
  {
    value: "WI - Badger State Auto Auction",
    label: "WI - Badger State Auto Auction",
  },
  {
    value: "WI - Greater Milwaukee Auto Auction",
    label: "WI - Greater Milwaukee Auto Auction",
  },
  {
    value: "WV - Capital City Auto Auction Inc",
    label: "WV - Capital City Auto Auction Inc",
  },
  {
    value: "WV - Mountain State Auto Auction",
    label: "WV - Mountain State Auto Auction",
  },
];
//
// End of Filter Input Options
